function loadForm() {
	$(".load-form").load(base_url + '/admin/kategori/form', function (r, s, x) {
		if (s === 'error') {
...
function validFile() {
	...
	if (ID.val()) {
		...
		if (ID.data('image')) {
			...
			new FileUploadWithPreview('imagesIcon', {
				presetFiles: [
					ID.data('image')
				],
			});
...
function remove(page) {
	$(document).on('click', '.hapus', function () {
		...
		var ID = $(this).data('id');
		var url = base_url + "/kategori/remove";
...
function loadFormEdit() {
	...
	$(document).on('click', '.edit', function () {
		...
		var ID = $(this).data('id');
		var url = base_url + "/admin/kategori/form";
		...
		$.ajax({
			...
			complete: function () {
				...
				UrlUp.attr('action', base_url + 'kategori/update');
				formAttribute();
				validFile();
			}
		});
...
function loadData() {
	$(".load-data").load(base_url + '/admin/kategori/data', function (r, s, x) {
		if (s === 'error') {
			console.log('Loaded failed!');
			return false;
		} else {
...
function loadFirstForm() {
	var UrlUp = $(".load-form").closest('.container-fluid').find('form').removeAttr('action');
	UrlUp.attr('action', base_url + 'kategori/store');
}
