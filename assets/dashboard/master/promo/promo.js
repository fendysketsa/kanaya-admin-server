function loadForm() {
	$(".load-form").load(base_url + '/admin/promo/form', function (r, s, x) {
		if (s === 'error') {
...
function loadFormEdit() {
	...
	$(document).on('click', '.edit', function () {
		...
		var ID = $(this).data('id');
		var url = base_url + "/admin/promo/form";
		...
		$.ajax({
			...
			complete: function () {
				...
				UrlUp.attr('action', base_url + 'promo/update');
				formAttribute();
				validFile();
			}
		});
...
function loadData() {
	$(".load-data").load(base_url + '/admin/promo/data', function (r, s, x) {
		if (s === 'error') {
			console.log('Loaded failed!');
			return false;
		} else {
...
function loadFirstForm() {
	var UrlUp = $(".load-form").closest('.container-fluid').find('form').removeAttr('action');
	UrlUp.attr('action', base_url + 'promo/store');
}
