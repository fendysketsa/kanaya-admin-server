function loadForm() {
	$(".load-form").load(base_url + '/admin/role/form', function (r, s, x) {
		if (s === 'error') {
...
function loadFormEdit() {
	...
	$(document).on('click', '.edit', function () {
		...
		var ID = $(this).data('id');
		var url = base_url + "/admin/role/form";
		...
		$.ajax({
			...
			complete: function () {
				...
				UrlUp.attr('action', base_url + 'role/update');
				formAttribute();
			}
		});
	});
...
function loadData() {
	$(".load-data").load(base_url + '/admin/role/data', function (r, s, x) {
		if (s === 'error') {
			console.log('Loaded failed!');
			return false;
		} else {
...
function loadFirstForm() {
	var UrlUp = $(".load-form").closest('.container-fluid').find('form').removeAttr('action');
	UrlUp.attr('action', base_url + 'role/store');
}
...
function submit() {
	...
	$('form#formRole').submit(function (e) {
		e.preventDefault();
		var elemt = $(this)[0];
