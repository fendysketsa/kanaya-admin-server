// load table
function loadData() {
	$(".load-data").load(base_url + '/admin/acc-receivable/data', function (r, s, x) {
		if (s === 'error') {
...
function dataAttribute() {
	...
	var table_ = $('#dataView').DataTable({
		...
		buttons: [{
			...
			className: 'text-left',
			autoFilter: true,
			filename: 'data_piutang',
			title: 'Laporan Piutang Produk',
			messageTop: function () {
				return "Tanggal: " + $("#range_date").attr('data')
			},
			sheetName: 'Data Piutang'
		}, ],
		"columns": [{
				'data': 'id',
